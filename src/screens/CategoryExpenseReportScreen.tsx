...
const CategoryExpenseReportScreen: React.FC = () => {
    ...
    const loadData = async () => {
        try {
            ...
            const data = await ReportService.getCategoryWiseExpense(startDate, endDate);
            setCategoryData(data);
            setTotalExpense(data.reduce((sum, item) => sum + item.total, 0));
        } catch (error) {
            console.error('Failed to load category expense report:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity
                    onPress={() => ExportHelper.exportReport('Spending Breakdown', categoryData)}
